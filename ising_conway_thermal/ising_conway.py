...
def metropolis_accept(sites, beta=0.1):
    total_energy = get_total_energy(sites)
    return np.min([1.0, np.exp(-beta * total_energy)]) < np.random.random()


def glauber_accept(sites, beta=0.1):
    total_energy = get_total_energy(sites)
    return np.min([1.0, 1.0 / (1.0 + np.exp(beta * total_energy))]) < np.random.random()


def move_step(sites, beta=0.1, dynamics_type="metropolis"):
    """Single site move."""
    if dynamics_type == "metropolis":
        is_accept = metropolis_accept(sites, beta)
    if dynamics_type == "glauber":
        is_accept = glauber_accept(sites, beta)
    if is_accept:
...
def play(
    M=10, N=3, nstep=1000, beta=0.1, dynamic_type="glauber", seed=42 * 42
):
    """Play the game"""
    np.random.seed(seed)
    ...
    for _ in np.arange(nstep):
        ...
        m_sites, omega_size = move_step(
            m_sites, beta=beta, dynamics_type=dynamic_type
        )
